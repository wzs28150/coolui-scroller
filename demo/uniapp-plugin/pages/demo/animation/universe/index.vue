...
import { ref } from 'vue'
import type { CooluiRefreshConfig, CooluiScrollerApi } from '@/uni_modules/coolui-scroller/types'
...
const onRefresh = () => {
  // 自行驱动刷新状态，确保下拉时 .loading 类一定生效（不依赖 refresh 组件的回传）
  refreshstate.value = 'loading'
  setTimeout(() => {
    // 开启第二段动画
    isLoading.value = true
    setTimeout(() => {
      universeScroller.value && universeScroller.value.settriggered(false)
      ...
      >
        <template #refresh>
          <coolui-scroller-refresh
            ...
            :config="universeSetting"
            @update:threshold="onThreshold"
            @update:refreshstate="onRefreshstate"
          >
            <view
              class="universe-wapper"
              :class="[refreshstate, isLoading ? 'loading2' : '']"
            >
              <view
                class="universe-bg"
                :style="`transform:translate(-50%, -50%) scale(${1 + 0.5 * val})`"
              ></view>
              <view
                class="earth-group"
                :style="`transform:translate(-50%, -50%) scale(${0.2 * val})`"
              >
                <view class="earth"></view>
                <view class="moon" :class="refreshstate"></view>
              </view>
              <view
                class="sun"
                :style="`transform:translate(${-100 + 100 * val}%, ${-100 +
                  100 * val}%) scale(${1 * val})`"
              ></view>
              <view
                class="jupiter"
                :class="refreshstate"
                :style="`transform: scale(${1 + 1 * val}) translate(${-100 *
                  val}%, 0%)`"
              ></view>
              <view
                class="ball"
                :style="`transform:translate(${100 - 100 * val * 4}%, ${-100 +
                  100 * val}%) scale(${1 * val})`"
              ></view>
            </view>
          </coolui-scroller-refresh>
