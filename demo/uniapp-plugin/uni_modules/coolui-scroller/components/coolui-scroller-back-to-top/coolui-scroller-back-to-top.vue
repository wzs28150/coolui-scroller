<template>
  <view class="coolui-backToTop" @tap="backToTop"></view>
</template>

<script setup lang="ts">
import { inject, onMounted, onBeforeUnmount } from 'vue'
import type { CooluiScrollerApi } from '../../types'

const props = defineProps({
  delay: {
    type: Number,
    default: 3000,
  },
  threshold: {
    type: Number,
    default: 100,
  },
})

const cooluiScroller = inject<CooluiScrollerApi | null>('cooluiScroller', null)

const backToTop = () => {
  if (cooluiScroller && cooluiScroller.scrollToTop) {
    cooluiScroller.scrollToTop()
  }
}

// scroller 的 scroll() 会读取 threshold / delay，需以普通对象暴露
const backToTopApi = {
  get threshold() {
    return props.threshold
  },
  get delay() {
    return props.delay
  },
}

onMounted(() => {
  if (cooluiScroller && cooluiScroller.registerBackToTop) {
    cooluiScroller.registerBackToTop(backToTopApi)
  }
})

onBeforeUnmount(() => {
  if (cooluiScroller && cooluiScroller.unregisterBackToTop) {
    cooluiScroller.unregisterBackToTop()
  }
})

// 父组件 scroll() 会读取 backToTopNode.threshold / .delay，需暴露到实例上
defineExpose({
  get threshold() {
    return props.threshold
  },
  get delay() {
    return props.delay
  },
  backToTop,
})
</script>

<style lang="scss">
.coolui-backToTop {
  background-color: rgba(0, 0, 0, 0.6);
  background-image: url('data:image/png;base64,');
  background-size: 45rpx;
  background-repeat: no-repeat;
  background-position: center;
  width: 80rpx;
  height: 80rpx;
  position: fixed;
  bottom: 40rpx;
  right: 30rpx;
  border-radius: 100%;
  z-index: 5;
  opacity: 0.6;
}
</style>
